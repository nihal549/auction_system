  ...
  async (dispatch) => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:5000/ad?option=notexpired`,
        config
      );
...
export const loadAdDetails = (adId) => async (dispatch) => {
  try {
    ...
    const res = await axios.get(`http://localhost:5000/ad/${adId}`);
...
export const loadHighestBid = (adId) => async (dispatch) => {
  const url = `http://localhost:5000/bid/${adId}`;
  try {
...
export const startAuction = (adId) => async (dispatch) => {
  const url = `http://localhost:5000/auction/start/${adId}`;
  try {
...
export const getUserPurchasedAds = () => async (dispatch) => {
  const url = `http://localhost:5000/user/products/purchased`;
  try {
    const res = await axios.get(url);
